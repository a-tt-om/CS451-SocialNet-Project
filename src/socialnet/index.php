...
if (isUserAdmin()) {
    ...
    $stmt = $pdo->prepare("SELECT id, username, fullname FROM account WHERE id != ? AND is_admin = 0 ORDER BY fullname ASC");
}
$stmt->execute([$currentUserId]);
                ...
                <ul class="user-list">
                    <?php foreach ($otherUsers as $user): ?>
                        <li class="user-list-item">
                            ...
                            <a href="/socialnet/profile.php?owner=<?= urlencode($user['username']) ?>" class="item-action">View
                                Profile</a>
                        </li>
                    <?php endforeach; ?>
                </ul>
